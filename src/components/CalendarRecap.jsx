        ...
        for (const planning of planningDetail['hydra:member']) {
          if (isSameDay(new Date(strTime), new Date(planning.date))) {
            ...
            try {
              ...
              // On s'assure que typeLabels et mealLabelsFetched ont la même longueur
              typeLabels.forEach((type, index) => {
                if (!type) {
                  console.warn('Type undefined found at index:', index);
                  return;
                }

                const typeLabel = type.toLowerCase();
                const mealLabel = mealLabelsFetched[index];

                if (!mealLabel) {
                  console.warn('Meal undefined found at index:', index);
                  return;
                }

                if (typeLabel.includes('matin')) {
                  newItems[strTime].mealsByTimeOfDay.matin.push(mealLabel);
                } else if (typeLabel.includes('midi')) {
                  newItems[strTime].mealsByTimeOfDay.midi.push(mealLabel);
                } else if (typeLabel.includes('soir')) {
                  newItems[strTime].mealsByTimeOfDay.soir.push(mealLabel);
                }
              });

              newItems[strTime].id = planning.id;
              newItems[strTime].name = 'Repas prévu';
            } catch (error) {
  ...
  const renderItem = (item, date) => {
    ...
    return (
      <Pressable
        ...
        onPress={() =>
          navigation.navigate('MealPlannerScreen', {userId, date, item})
        }>
        {noMealPlanned ? (
